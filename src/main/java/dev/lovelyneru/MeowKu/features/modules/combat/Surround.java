...
extends Module {
    ...
    private final Setting<Double> safehealth = this.register(new Setting<Double>("Safe Health", Double.valueOf(12.5), Double.valueOf(1.0), Double.valueOf(36.0), v -> this.breakcrystal.getValue()));
    ...
    @Override
    public void onTick() {
        if (this.breakcrystal.getValue().booleanValue() && (double)Surround.mc.player.getHealth() >= this.safehealth.getValue() && this.isSafe == 0) {
            this.breakcrystal();
        }
        this.doFeetPlace();
    }
        ...
        block5: for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, false)) {
                case 1: {
                    ...
                    if (MeowKu.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= 1) continue block5;
