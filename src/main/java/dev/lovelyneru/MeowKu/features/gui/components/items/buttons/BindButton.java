...
public class BindButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515) : (!this.isHovering(mouseX, mouseY) ? MeowKu.colorManager.getColorWithAlpha(MeowKu.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : MeowKu.colorManager.getColorWithAlpha(MeowKu.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())));
        if (this.isListening) {
            MeowKu.textManager.drawStringWithShadow("Press a Key...", this.x + 2.3f, this.y - 1.7f - (float) MeowKuGui.getClickGui().getTextOffset(), -1);
        } else {
            MeowKu.textManager.drawStringWithShadow(this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getValue().toString().toUpperCase(), this.x + 2.3f, this.y - 1.7f - (float) MeowKuGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
