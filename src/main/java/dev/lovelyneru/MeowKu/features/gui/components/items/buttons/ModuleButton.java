...
public class ModuleButton
    ...
    private final ResourceLocation logo = new ResourceLocation("textures/Icon.png");
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            if (HUD.getInstance().magenDavid.getValue().booleanValue()) {
                ...
                ModuleButton.drawCompleteImage(this.x - 1.5f + (float) this.width - 7.4f, this.y - 2.2f - (float) MeowKuGui.getClickGui().getTextOffset(), 8, 8);
            }
            if (this.subOpen) {
                float height = 1.0f;
