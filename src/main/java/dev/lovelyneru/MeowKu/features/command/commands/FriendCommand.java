...
import dev.lovelyneru.MeowKu.manager.FriendManager;
...
public class FriendCommand
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (MeowKu.friendManager.getFriends().isEmpty()) {
                FriendCommand.sendMessage("Friend list empty D:.");
            } else {
                String f = "Friends: ";
                for (FriendManager.Friend friend : MeowKu.friendManager.getFriends()) {
                    try {
        ...
        if (commands.length == 2) {
            ...
            FriendCommand.sendMessage(commands[0] + (MeowKu.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
                case "add": {
                    MeowKu.friendManager.addFriend(commands[1]);
                    FriendCommand.sendMessage(ChatFormatting.GREEN + commands[1] + " has been friended");
                    return;
                }
                case "del": {
                    MeowKu.friendManager.removeFriend(commands[1]);
