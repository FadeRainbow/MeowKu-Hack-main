...
import dev.lovelyneru.MeowKu.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "MeowKu/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        MeowKu.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    MeowKu.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("MeowKu").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("MeowKu/" + name + "/"))) {
            this.config = "MeowKu/" + name + "/";
        } else {
            this.config = "MeowKu/config/";
        }
        MeowKu.friendManager.onLoad();
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("MeowKu").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("MeowKu/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        MeowKu.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("MeowKu/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("MeowKu", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(MeowKu.moduleManager.modules);
        this.features.add(MeowKu.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        MeowKu.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            MeowKu.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
